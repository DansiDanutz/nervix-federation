-- Migration 004: Update submissions table schema
-- This recreates the submissions table with the correct schema

-- Drop existing submissions table (data should be minimal)
DROP TABLE IF EXISTS submissions CASCADE;

-- Drop existing quality_reviews table (references submissions)
DROP TABLE IF EXISTS quality_reviews CASCADE;

-- Create task_submissions table with new schema
...
DROP TRIGGER IF EXISTS task_submission_status_trigger ON task_submissions;
...
DROP VIEW IF EXISTS agent_submission_stats;
